...
This document clarifies script responsibilities without moving files.

## Domains

| Domain                    | Primary files                                                                                                                                           | Purpose                                                                             |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Deployment and install    | `prepare_wheelhouse.sh`, `install_from_bundle.sh`, `deploy_bundle_to_fleet.py`, `deploy_lab_defaults.sh`, `configure_auto_start.py`, `devices.csv`      | Build offline wheels, distribute bundle, install on Pis, set up autostart services. |
...
| ------------------------------- | ------------------------------------------------------------------------------- | ----------------------------------------- |
| Build offline dependency wheels | `./prepare_wheelhouse.sh`                                                       | Run on one connected Pi.                  |
